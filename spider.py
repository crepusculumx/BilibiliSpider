...
class Spider(threading.Thread):
    ...
    def login(self) -> None:
        ...
        with self.account_p_lock:
            if self.account_p == bilibili_accounts:
                self.account_p = 0
                print("b站账户不足以每个线程分配一个，将分配一个账户给多个线程")
            username = bilibili_accounts[self.account_p]["username"]
            password = bilibili_accounts[self.account_p]["password"]
            self.account_p += 1
